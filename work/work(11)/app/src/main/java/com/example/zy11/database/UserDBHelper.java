package com.example.zy11.database;
...
import com.example.zy11.bean.UserInfo;
...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String create_sql = "CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
                ...
                + "phonepwd  VARCHAR NOT NULL," + "update_time VARCHAR NOT NULL"
...
// 如果存在同样的手机号码，则更新记录
            ...
            cv.put("phonenumber", info.phonenumber);
            cv.put("phonepwd", info.phonepwd);
    ...
    public ArrayList<UserInfo> query(String condition) {
        String sql = String.format("select rowid,name,phonenumber,phonepwd,update_time," +
        ...
        while (cursor.moveToNext()) {
            ...
            info.phonenumber= cursor.getString(2); // 取出字符串
            info.phonepwd = cursor.getString(3); // 取出字符串

            //SQLite没有布尔型，用0表示false，用1表示true
            info.update_time = cursor.getString(4);
            info.phone = cursor.getString(5);
            info.pwd = cursor.getString(6);
            infoArray.add(info);
        }
    ...
    public UserInfo queryByPhone(String phonenumber) {
        UserInfo info = null;
        ArrayList<UserInfo> infoArray = query(String.format("phonenumber='%s'",phonenumber));
        if (infoArray.size() > 0) {
